...
import { Button, Header,IconButton,TransactionItem,Section} from "../../components";
...
const TransferDashboard = ({navigation}) => {
    ...
    const getData = async () => {
        ...
        setIncomingTransactions(dummyData.Transaction.filter((x) => x.type =='input'));
        setOutgoingTransactions(dummyData.Transaction.filter((x) => x.type =='output'));
    ...
    function renderBalance (){
        return(
            ...
            {balance?(
                ...
                <Text style={styles.noBalance} numberOfLines={2} adjustsFontSizeToFit  >{t('noBalance')}</Text>
                
                <Button label={t('depositNow')} 
                containerStyle={{marginTop:20}}
                labelStyle={{color:COLORS.darkBlue2,...FONTS.h3, fontSize:18}}
                icon={icons.right_arrow} 
                iconPosition={'RIGHT'}
                iconStyle={{width:16, height:16, marginLeft:10, tintColor:COLORS.darkBlue2}}/>
                </View>
    ...
    function renderOptions(){
        return(
            <View style={{marginTop:48, ...SIZES.marginHorizontal, 
              flexDirection:"row", justifyContent:"space-between", alignItems:"center"}}>
                ...
                <IconButton 
                containerStyle={{...styles.IconButtonContainer, backgroundColor:'#B548C6'}}
                icon={icons.transfer}
                iconStyle={{...styles.IconButtonIcon}} />
                ...
                <IconButton 
                containerStyle={{...styles.IconButtonContainer, backgroundColor:'#FF8700'}}
                icon={icons.transfer}
                iconStyle={{...styles.IconButtonIcon}} />
                ...
                <IconButton 
                containerStyle={{...styles.IconButtonContainer, backgroundColor:'#22B07D'}}
                icon={icons.more}
                iconStyle={{...styles.IconButtonIcon}} />
        ...
        return(
            <View>
                 ...
                 {outgoingTansaction.slice(0,2 ).map((item,index)=>{
                    return( 
                    <TransactionItem  item={item} onPress={()=> navigation.navigate('TransactionDetail', { item })}
                    containerStyle={{...styles.transactionItemContainer, marginBottom:index==1?80:null}}/>
                    )
                })}
