...
import { Header,SearchBar,Section,ContactItem, Button} from "../../components";
import Modal from "react-native-modal";
import { ScrollView } from "react-native-gesture-handler";

 const SelectContactModel = ({isVisible,closeModel,getSelectedContact}) =>{
  ...
  //code will run every time searchphrase is changed and fliter data base on search 
    const filterContact =  useMemo(() =>{
        ...
        // if search is different then blank
         ...
         if(searchPhrase.trim() != ""){
          setFilterContacts( allContacts.filter((x) => x.name.toUpperCase().startsWith(searchPhrase.toUpperCase().trim().replace(/\s/g, ""))));
    ...
    function renderSearchBar(){
        return(
            <View style={{...SIZES.marginHorizontal, justifyContent:"center",alignItems:"center", marginTop:44}}>
             <SearchBar
    ...
    function renderContacts(){
        return(
            <View style={styles.listsContainer}>
                ...
                {recentContacts && 
                <View>
                <Section label={t('recentsContacts')} containerStyle={{marginHorizontal:0}}/>
                {recentContacts?.map((item) =>{
                    return(
                        <ContactItem item={item}
                         //isSelected={(selectedContact?.key == 'recent' && selectedContact?.id== item.id)}
                        onPress={()=>  getSelectedContact(item)}//on parent useCallBack is triggered with selected item as parm
                        />
                    )
                })}
                </View>
                ...
                {allContacts ?(
                 ...
                 <Section label={t('allContacts')} containerStyle={{marginHorizontal:0, paddingBottom:12}}/>
